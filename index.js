...
hdr.load().then(() => {
  console.log(hdr.getImageData(0, 0));
  const canvas = new Canvas("canvas");
  const camera = new Camera(canvas, 90, new Vector3(0, 0, -5), Infinity, hdr);
  ...
  {
    const position = new Vector3(0, -22, 0);
    const radius = 20;
    const color = new Vector3(255, 255, 255);
    floor = new Sphere(radius, position, new Material(color, -1, 0.9));
    scene.add(floor);
  }

  // scene.add(new PointLight(new Vector3(0, 5, -5), 2  ));
  scene.add(new PointLight(new Vector3(0, 5, -5), 5));
  ...
  const render = () => {
    ...
    camera.position = camera.position.add(
      input.scale(delta * 0.016 * CAMERA_SPEED).rotate(camera.rotation.y, camera.rotation.x)
    );
